...
import type { NovaTermLspClient } from "./client";
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { NovaTermLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (NovaTermLspClient.hostPid === null) {
    NovaTermLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
  ...
  const client = new NovaTermLspClient({
    transport,
    rootUri,

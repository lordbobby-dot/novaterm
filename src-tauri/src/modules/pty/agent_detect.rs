...
// OSC 777 marker our agent hooks emit. Legacy 3-field `notify;NovaTerm;<event>`
// (Claude) or 4-field `notify;NovaTerm;<agent>;<event>` (Codex/Gemini).
const NOVATERM_MARKER: &[u8] = b"notify;NovaTerm;";
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(tail) = pt.strip_prefix(NOVATERM_MARKER) {
...
mod tests {
    ...
    #[test]
    fn novaterm_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;NovaTerm;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;NovaTerm;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;NovaTerm;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;NovaTerm;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn novaterm_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;NovaTerm;attention")),
    ...
    fn four_field_marker_self_arms_named_agent() {
        ...
        let mut d = AgentDetector::new();
        assert_eq!(run(&mut d, &osc("777;notify;NovaTerm;codex;working")), vec![started("codex")]);
        let mut g = AgentDetector::new();
        assert_eq!(
            run(&mut g, &osc("777;notify;NovaTerm;gemini;finished")),
    ...
    fn four_field_marker_ignores_unknown_agent() {
        let mut d = AgentDetector::new();
        assert!(run(&mut d, &osc("777;notify;NovaTerm;evil;attention")).is_empty());
        // A known agent in the same chunk still works.
        assert_eq!(
            run(&mut d, &osc("777;notify;NovaTerm;codex;attention")),
    ...
    fn four_field_marker_drives_status_after_preexec() {
        ...
        assert_eq!(run(&mut d, &osc("777;notify;NovaTerm;gemini;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;NovaTerm;gemini;working")), vec![Transition::Working]);
        assert_eq!(run(&mut d, &osc("777;notify;NovaTerm;gemini;finished")), vec![Transition::Finished]);
    }

# novaterm-shell-integration (PowerShell)
...
if ($global:__NOVATERM_HOOKS_LOADED) { return }
$global:__NOVATERM_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__novaterm_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__novaterm_urlencode {
    param([string]$s)
...
function global:__novaterm_install_readline {
    if ($global:__novaterm_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__novaterm_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__novaterm_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __novaterm_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__novaterm_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__novaterm_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($loc.Provider.Name -eq 'FileSystem') {
        ...
        $cwdEnc = __novaterm_urlencode $cwd
    ...
    if ($env:NOVATERM_BLOCKS) {
        $global:LASTEXITCODE = $lec
        $gap = if ($global:__novaterm_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__novaterm_user_prompt) {
        try { & __novaterm_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {

# novaterm-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_novaterm_ret`.

{
  _novaterm_user_zdotdir="${NOVATERM_USER_ZDOTDIR:-$HOME}"
  [ -f "$_novaterm_user_zdotdir/.zshrc" ] && source "$_novaterm_user_zdotdir/.zshrc"
...
if [[ -z "$__NOVATERM_HOOKS_LOADED" ]]; then
  ...
  _novaterm_precmd() {
    local _novaterm_ret=$?
    printf '\e]133;D;%s\e\\' "$_novaterm_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_novaterm_urlencode "$PWD")"
    ...
    if [[ -n "$NOVATERM_BLOCKS" ]]; then
      ...
      if [[ -n "$_novaterm_block_seen" ]]; then
        PS1=$'\n\n%{\e]133;B\e\\%}'
      else
        PS1=$'\n%{\e]133;B\e\\%}'
      fi
  ...
  _novaterm_preexec() {
    ...
    [[ -n "$NOVATERM_BLOCKS" ]] && _novaterm_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"

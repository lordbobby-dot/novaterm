# novaterm-shell-integration (bashrc)
#
...
if [ -z "$__NOVATERM_HOOKS_LOADED" ]; then
  ...
  _novaterm_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_novaterm_urlencode "$PWD")"
    ...
    elif [ -z "$__NOVATERM_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __NOVATERM_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  if [ "${BASH_VERSINFO[0]:-0}" -gt 4 ] \
    ...
    if [ -n "$NOVATERM_BLOCKS" ]; then
      ...
      PS0='\[\e]133;C\e\\\]${_novaterm_noop[$((_novaterm_block_seen=1))]}'"${PS0:-}"
    else
      PS0='\[\e]133;C\e\\\]'"${PS0:-}"
    fi
  fi

...
const OWNED_MARKERS: [&str; 3] = ["notify;NovaTerm;", "novaterm;notify", "__novaterm_notify"];
...
fn hook_command(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!(
            r#"[ -n "$NOVATERM_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;NovaTerm;{event}\\u0007"}}' || true"#
        ),
...
fn osc_command(agent: &str, event: &str) -> String {
    format!(
        r#"[ -n "$NOVATERM_TERMINAL" ] && printf '\033]777;notify;NovaTerm;{agent};{event}\007' > /dev/tty; printf '{{}}'"#
    )
}
...
fn osc_command(agent: &str, event: &str) -> String {
    let exe = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "novaterm.exe".to_string());
    format!(r#""{exe}" __novaterm_notify {agent} {event}"#)
}
...
fn status_needle(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!("notify;NovaTerm;{event}"),
        Delivery::Osc => {
            #[cfg(unix)]
            {
                format!("notify;NovaTerm;{};{event}", spec.agent)
            }
            #[cfg(windows)]
            {
                format!("__novaterm_notify {} {event}", spec.agent)
            }
        }
    }
}
...
pub fn agent_enable_hooks(agent: String) -> Result<(), String> {
    ...
    // a truncated config.
    let tmp = path.with_extension("novaterm-tmp");
...
fn conout_marker(agent: &str, event: &str) -> String {
    format!("\x1b]777;notify;NovaTerm;{agent};{event}\x07")
}

// Windows has no /dev/tty: the hook calls `novaterm.exe __novaterm_notify ...` and we
...
pub fn emit_conout_marker(agent: &str, event: &str) {
    ...
    if std::env::var_os("NOVATERM_TERMINAL").is_none() {
        return;
    }
    unsafe {
...
mod tests {
    ...
    fn claude_adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;NovaTerm;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;NovaTerm;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;NovaTerm;working"));
    ...
    fn conout_marker_matches_detector_format() {
        ...
        assert_eq!(
            ...
            "\u{1b}]777;notify;NovaTerm;gemini;attention\u{7}"
        );
    }
    ...
    fn codex_emits_four_field_dev_tty_marker() {
        ...
        assert!(stop.contains("notify;NovaTerm;codex;finished"));
    ...
    fn gemini_uses_matcher_and_named_marker() {
        ...
        assert!(command(&out, "AfterAgent", 0).contains("notify;NovaTerm;gemini;finished"));
        assert!(command(&out, "Notification", 0).contains("notify;NovaTerm;gemini;attention"));
    }
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$NOVATERM_TERMINAL\" ] && printf '\\033]777;novaterm;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
